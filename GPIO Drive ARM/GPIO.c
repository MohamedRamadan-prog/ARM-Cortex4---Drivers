...
GPIO_CheckType GPIO_Read(uint8_t GroupId,uint8_t* GroupDataPtr)
{
	...
	const GPIO_CfgType * CfgRed;
		GPIO_CheckType RetVar;
		if (( GroupId <GPIO_GROUPS_NUMBER))
			{

			CfgRed =  & GPIO_ConfigParam[GroupId];

				if ((CfgRed->PortDirection == 0x00)&&(CfgRed->SetPinType == 0xff)&&(CfgRed->UseAlterFun==0x00) &&(GPIO_GroupState[GroupId]==1 ))
				{

					*GroupDataPtr = GPIODATA_READ(CfgRed->PortMask,CfgRed->PortId);
